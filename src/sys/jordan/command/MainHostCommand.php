...
class MainHostCommand extends BaseCommand{
					...
					if(isset($args[1])){
						...
						if($args[1] === "size"){
							if(isset($args[2]) and is_numeric($args[2])){
								if($args[2] < 2) return $sender->sendMessage(TextFormat::RED."The minimum team size is 2!");
								if($args[2] > 25) return $sender->sendMessage(TextFormat::RED."The maximum team size is 25!");
								TeamManager::$MAX_TEAM_COUNT = $args[2];
								return $sender->sendMessage(TextFormat::GREEN."The team count has been set to ".$args[2]."!");
							}
						}
						if($args[1] === "true"){
	...
	public function sendHostHelp(CommandSender $sender){
		$sender->sendMessage(TextFormat::GRAY."--Hosting Help--");
		...
		$sender->sendMessage(TextFormat::GOLD."teams: ".TextFormat::GRAY."Set team options. Usage: /host teams [size|true|false] (size-count)");

...
class ParadoxPlayer extends Player {
	...
	public function __construct(SourceInterface $interface, $clientID, $ip, $port) {
		...
		if(($plugin = $this->getServer()->getPluginManager()->getPlugin("UHC")) instanceof Central and $plugin->isEnabled()) {
			$this->plugin = $plugin;
		} else {
	...
	public function loadStats(){
		if(!file_exists($this->getPlugin()->getDataFolder()."players/".strtolower($this->getName()).".stats")) {
			$this->statFile = new Config($this->getPlugin()->getDataFolder()."players/".strtolower($this->getName()).".stats", Config::JSON, array(
				"Kills"=> 0,
				"Deaths"=> 0,
				"Wins"=> 0,
				"Heads"=> 0,

...
class MainListener implements Listener{
	...
	public function onBreak(BlockBreakEvent $event){
		...
		if($player instanceof ParadoxPlayer and $this->eventChecker($player)) $event->setCancelled();
	}
	...
	public function onJoin(PlayerJoinEvent $event){
		...
		if($player instanceof ParadoxPlayer) {
			$player->loadStats();
			if($this->plugin->getTeamManager()->isTeamsEnabled() and $player->inTeam()){
				$player->setDisplayName($player->getTeam()->getTeamString().TextFormat::GRAY.$player->getDisplayName());
			}
	...
	public function onChange(EventChangeEvent $event){
		switch($event->getAfterEvent()){
			case EventManager::COUNTDOWN:
				if(!$this->plugin->getEssentialManager()->getGlobalMute()) {
